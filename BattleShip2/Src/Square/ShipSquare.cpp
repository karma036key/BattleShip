...
void ShipSquare::Exec()
{
	...
	{
	case ShipID::SHIP2:
		if ((150 < Input::GetMousePosX() && Input::GetMousePosX() < 150 + (33 * 10)) && (20 < Input::GetMousePosY() && Input::GetMousePosY() < 20 + (33 * 10))			//カーソルが枠内にあるか
			&& ((GetDragPosXForDraw(GetCenterPosX()) - 16) < Input::GetDragMousePosX() && Input::GetDragMousePosX() < GetDragPosXForDraw(GetCenterPosX()) + 16) 		//			画像の横幅内にあるか
			&& ((GetDragPosYForDraw(GetCenterPosY()) - 32) < Input::GetDragMousePosY() && Input::GetDragMousePosY() < GetDragPosYForDraw(GetCenterPosY()) + 32)			//			画像の縦幅内にあるか
			&& GetIsHold())																																				//			画像内でクリックされたか
		{
			isDrag = true;
			DetectShipArea();
			SetCenterPosX();
